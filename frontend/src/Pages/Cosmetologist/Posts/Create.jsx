...
export default function Create({ products }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('cosmetologist.posts.store'));
    };
    ...
    return (
        <BeautyLayout>
            <Head title="Написать статью — Beauty Atelier" />
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
                <div className="mb-12">
                    ...
                    <h1 className="font-serif italic text-6xl text-deep-espresso">Новый пост</h1>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-12">
                    <div className="space-y-8">
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full bg-[#FAF9F6] border-none p-6 font-serif text-2xl focus:ring-1 focus:ring-champagne-gold transition-all"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <select 
                                    ...
                                    className="w-full bg-[#FAF9F6] border-none p-4 text-xs uppercase tracking-widest font-bold focus:ring-1 focus:ring-champagne-gold"
                                >
                            ...
                            <div>
                                <label className="block text-[10px] uppercase tracking-widest font-bold text-deep-espresso/40 mb-3">Изображение</label>
                                <input 
                                    type="file"
                                    onChange={e => setData('image', e.target.files[0])}
                                    className="w-full text-xs text-gray-500 file:mr-4 file:py-4 file:px-8 file:border-0 file:text-[10px] file:uppercase file:tracking-widest file:font-bold file:bg-champagne-gold/10 file:text-champagne-gold hover:file:bg-champagne-gold/20"
                                />
                            </div>
                        </div>

                        <div>
                            <label className="block text-[10px] uppercase tracking-widest font-bold text-deep-espresso/40 mb-3">Краткое описание (для анонса)</label>
                            <textarea 
                                value={data.excerpt}
                                onChange={e => setData('excerpt', e.target.value)}
                                className="w-full bg-[#FAF9F6] border-none p-6 text-sm font-light leading-relaxed h-32 focus:ring-1 focus:ring-champagne-gold"
                    ...
                    >
                        {processing ? 'Публикация...' : 'Опубликовать статью'}
                    </button>
                </form>
            </div>
