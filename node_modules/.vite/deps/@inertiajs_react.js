...
var createValidator = (callback, initialData = {}) => {
    ...
    return {
      ...
      onValidationError: (response, axiosError) => {
        [
          ...
          ...setErrors(merge_default(omit_default({ ...errors }, only), response.data.errors))
      ...
      onPrecognitionSuccess: (response) => {
        [
          ...setValidated([...validated, ...only]),
          ...setErrors(omit_default({ ...errors }, only))
        ].forEach((listener) => listener());
        return config3.onPrecognitionSuccess ? config3.onPrecognitionSuccess(response) : response;
      },
      onBefore: () => {
        if (config3.onBeforeValidation && config3.onBeforeValidation({ data, touched }, { data: oldData, touched: oldTouched }) === false) {
          return false;
        }
  ...
  const validate = (name, value, config3) => {
    ...
    if (get_default(oldData, name) !== value) {
...
function useForm(...args) {
  ...
  const validate = (field, config22) => {
    if (typeof field === "object" && !("target" in field)) {
      config22 = field;
      field = void 0;
    }
    if (field === void 0) {
      validatorRef.current.validate(config22);
    } else {
      const fieldName = resolveName(field);
      const currentData = dataRef.current;
      const transformedData = transform2.current(currentData);
      validatorRef.current.validate(fieldName, get_default(transformedData, fieldName), config22);
    }
    return form;
  };
  const withPrecognition = (...args2) => {
    ...
    const precognitiveForm = Object.assign(form, {
      ...
      invalid,
      touched,
      withoutFileValidation: () => tap2(precognitiveForm, () => validatorRef.current?.withoutFileValidation()),
      touch: (field, ...fields) => {
      ...
      setValidationTimeout: (duration) => tap2(precognitiveForm, () => validatorRef.current?.setTimeout(duration)),
      validateFiles: () => tap2(precognitiveForm, () => validatorRef.current?.validateFiles()),
      validate,
...
var Form = (0, import_react8.forwardRef)(
  ({
    ...
    const exposed = {
      ...
      validate: (field, config22) => form.validate(...UseFormUtils.mergeHeadersForValidation(field, config22, headers)),
      touch: form.touch,
